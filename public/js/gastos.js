...
const lista1= document.getElementById('listaPaga1');

const btnPagar = document.getElementById('btnPagar');
const btnEditar = document.getElementById('btnEditar');

const ListItems = document.querySelectorAll('.list-group-item');
const textGastos = document.getElementById('textGastos');
...
async function ObtenerRoommates(){

    const resultado = await fetch('/roommates');
    const conmpaneros = await resultado.json();

    conmpaneros.sort((a, b) => a.nombre.localeCompare(b.nombre));

    let fila ='<option selected disabled>Seleccione un nombre</option>';    

    conmpaneros.forEach( (rommy,index) => {
          fila += `<option value="${ rommy.id }">${ rommy.nombre }</option>`;
    });

    select1.innerHTML = fila;

};

// AGREGAR UN NUEV COSTO
async function AgregarGasto(id, nombre, descripcion, monto){

    try {

        fetch('/gastos',{ method : 'POST',
              ...
              window.location.href = '/agregargastos';  

        })
        
    } catch (error) {

        console.log(`X Error al registrar el gasto : `, error);
...
async function ObtenerGastos() {
    ...
    const resultado = await fetch('/gastos');
   
    const gastos = await resultado.json();
    
    let fila = '';
    gastos.forEach(gasto =>{    
       
        let idCorto = gasto.id;       
        ...
        fila += `<tr>
                    ...
                    <td>${ convertirMoneda(gasto.monto) }</td>
                    <td>
                        <button class="btnOpciones" type="button" onclick="(editGasto('${ gasto.idg }','${ gasto.nombre }','${ gasto.descripcion }','${ gasto.monto }'))">
                        ...
                        <button class="btnOpciones" type="button" onclick="(EliminarGasto('${ gasto.idg }'))">
...
async function EliminarGasto(id){    
    ...
    console.log(`id Eliminar: `,id)
     ...
     try {
        ...
        console.log(`X Error al eliminar gasto : `, error);
...
// EDITAR UN GASTO
async function editGasto(idg, nombre, descripcion, monto) {  
   ...
   btnEditar.style.display = 'block';
    ...
    let fila;
    fila += `<option value="${idg} selected disabled">${nombre }</option>`;

    select1.innerHTML = fila;

    textGastos.textContent = descripcion;
    aPagar.value = monto;
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    ListItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            let descGAstos = textGastos.value;            
            descGAstos += `${this.textContent} - `            
            textGastos.value = descGAstos;
    ...
    btnPagar.addEventListener('click',async()=>{
        ...
        descGAstos = descGAstos.slice( 0,descGAstos.length - 2) 

        let nombre = select.options[select.selectedIndex].text;

            await AgregarGasto (select.value , nombre, descGAstos , aPagar.value)        
    ...
    btnEditar.addEventListener('click',()=>{
            ...
            let nombre = select1.options[select1.selectedIndex].text;
            let descGAstos = textGastos.value;
            let MontoApagar = aPagar.value;
